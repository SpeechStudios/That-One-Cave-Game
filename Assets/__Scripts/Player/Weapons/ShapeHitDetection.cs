...
public class ShapeHitDetection : NetworkBehaviour
{
    ...
    private class PendingCast
    {
        ...
        public PreciseTick StartTick; // server only
    ...
    public void Cancel(int castId, bool isServer)
    {
        List<PendingCast> list = isServer ? ServerPending : ClientPending;
        ...
        if (isServer && ServerPending.Count == 0)
            TimeManager.OnTick -= Server_OnTick;
        else if (!isServer && ClientPending.Count == 0)
            TimeManager.OnTick -= Client_OnTick;
    }
    ...
    private int CastOverlap(ShapeType shape, Vector3 p1, Vector3 p2, float radius,  Collider[] buffer)
    {
        return shape switch
        {
            ShapeType.Sphere => Physics.OverlapSphereNonAlloc(p1, radius, buffer, Loadout.HitLayers),
            ShapeType.Capsule => Physics.OverlapCapsuleNonAlloc(p1, p2, radius, buffer, Loadout.HitLayers),
            _ => 0
        };
    }
